(function() {
    setTimeout(function() {
        ...
        const PLUGIN_ID = 'optimized_buttons_clear_cache';

        // Проверка на уже загруженный плагин
        if (window[PLUGIN_ID + '_loaded']) {
            console.log('Плагин уже загружен, пропускаем');
            return;
        }
        ...
        // Додавання CSS только если еще не добавлен
        ...
        // Оновлення кнопок с защитой от дублирования
        function updateButtons() {
            // Проверяем, не обрабатывались ли уже эти кнопки
            if ($('.full-start__button[data-optimized="true"]').length > 0) {
                console.log('Кнопки уже оптимизированы, пропускаем');
                return;
            }

            // Добавляем маркер обработки
            $('.full-start__button').attr('data-optimized', 'true');

            // Обновляем только если кнопки еще не имеют правильных иконок
            $('.full-start__button.view--torrent svg:not([data-replaced])').each(function() {
                $(this).attr('data-replaced', 'true').replaceWith(`
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px" data-replaced="true">
            ...
            $('.full-start__button.view--online svg:not([data-replaced])').each(function() {
                $(this).attr('data-replaced', 'true').replaceWith(`
            ...
            $('.full-start__button.view--trailer svg:not([data-replaced])').each(function() {
                $(this).attr('data-replaced', 'true').replaceWith(`
                    <svg height="70" viewBox="0 0 80 70" fill="none" xmlns="http://www.w3.org/2000/svg" data-replaced="true">
        ...
        if (window.plugin) {
            window.plugin('clear_cache_plugin', {
                type: 'component',
                name: 'Оптимізовані кнопки + очистка кешу',
                version: '2.4.0',
                author: 'Oleksandr',
                description: 'Фікс дублювання + стандартні анімації + синя кнопка Стрічка'
            });
        }
