    ...
    setTimeout(function() {
        ...
        if (!document.getElementById('clearcache-style')) {
            const css = `
                ...
                .full-start__button.view--online:not(.updated-icon) svg path {
                    fill: #2196f3 !important;
                }
                .full-start__button.view--torrent:not(.updated-icon) svg path {
                    fill: lime !important;
                }
                .full-start__button.view--trailer:not(.updated-icon) svg path {
        ...
        // Оновлення кнопок - ВИПРАВЛЕНА ВЕРСІЯ
        function updateButtons() {
            // Оновлюємо іконки тільки якщо вони ще не оновлені
            $('.full-start__button.view--torrent:not(.updated-icon)').each(function() {
                $(this).addClass('updated-icon').find('svg').replaceWith(`
            ...
            $('.full-start__button.view--online:not(.updated-icon)').each(function() {
                $(this).addClass('updated-icon').find('svg').replaceWith(`
            ...
            $('.full-start__button.view--trailer:not(.updated-icon)').each(function() {
                $(this).addClass('updated-icon').find('svg').replaceWith(`
        ...
        if (window.plugin) {
            window.plugin('clear_cache_plugin', {
                ...
                version: '2.3.2',
                author: 'Oleksandr',
                description: 'Фікс дублювання + стандартні анімації'
            });
        }
