(function() {
    ...
    setTimeout(function() {
        ...
        // CSS стили + скрытие дублирующих источников
        if (!document.getElementById('clearcache-style')) {
            const css = `
                ...
                /* Скрыть дублирующие источники */
                .filter--menu .scroll__content > div:not(:first-child).selector[data-source]:has(.filter--menu__title:contains("Онлайн")) {
                    display: none !important;
                }
                
                /* Альтернативный способ - скрыть по тексту */
                .filter--menu .scroll__content > div.selector:has(.filter--menu__title):has(.filter--menu__subtitle:contains("Lampac")) {
                    display: none !important;
                }
        ...
        // Функция скрытия дублирующих источников
        function hideDuplicateSources() {
            setTimeout(() => {
                // Найти все источники "Онлайн" и оставить только первый (ваш UAflix)
                $('.filter--menu .scroll__content > div.selector').each(function(index) {
                    const $this = $(this);
                    const titleText = $this.find('.filter--menu__title').text().trim();
                    const subtitleText = $this.find('.filter--menu__subtitle').text().trim();
                    
                    // Скрыть источники содержащие "Lampac" или "Online" (но не первый)
                    if ((titleText.includes('Онлайн') || subtitleText.includes('Lampac')) && index > 0) {
                        // Проверить что это не ваш UAflix
                        if (!subtitleText.includes('UAflix') && !titleText.includes('UAflix')) {
                            $this.hide();
                            console.log('Скрыт дублирующий источник:', titleText, subtitleText);
                        }
                    }
                });
            }, 500);
        ...
        // Обновление кнопок + скрытие дублей
        function updateButtons() {
            setTimeout(() => {
                $('.full-start__button.view--torrent:not(.icon-updated)').each(function() {
                    $(this).addClass('icon-updated').find('svg').replaceWith(`
                ...
                $('.full-start__button.view--online:not(.icon-updated)').each(function() {
                    $(this).addClass('icon-updated').find('svg').replaceWith(`
                ...
                $('.full-start__button.view--trailer:not(.icon-updated)').each(function() {
                    $(this).addClass('icon-updated').find('svg').replaceWith(`
                ...
                // Скрыть дублирующие источники
                hideDuplicateSources();
            }, 1000);
        }

        // ТОЛЬКО однократный вызов
        updateButtons();

        // Регистрация плагина
        if (window.plugin) {
            window.plugin('clean_sources_plugin', {
                type: 'component',
                name: 'Скрытие дублей источников',
                version: '4.0.0',
                author: 'Oleksandr',
                description: 'Автоматически скрывает дублирующие онлайн-источники'
            });
        }
