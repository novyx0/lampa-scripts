(function() {
    setTimeout(function() {
        ...
        // Исправленная функция обновления кнопок
        function updateButtons() {
            // Проверяем, не были ли кнопки уже обновлены
            if ($('.full-start__button.view--torrent svg[data-updated]').length > 0) {
                return; // Кнопки уже обновлены, выходим
            }

            $('.full-start__button.view--torrent svg').each(function() {
                if (!$(this).attr('data-updated')) {
                    $(this).replaceWith(`
                        <svg data-updated="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">
            ...
            $('.full-start__button.view--online svg').each(function() {
                if (!$(this).attr('data-updated')) {
                    $(this).replaceWith(`
                        <svg data-updated="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
            ...
            $('.full-start__button.view--trailer svg').each(function() {
                if (!$(this).attr('data-updated')) {
                    $(this).replaceWith(`
                        <svg data-updated="true" height="70" viewBox="0 0 80 70" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        // Следим за изменениями только один раз
        let listenerAdded = false;
        if (Lampa && Lampa.Listener && !listenerAdded) {
        ...
        window.plugin && window.plugin('clear_cache_plugin', {
            ...
            version: '2.3.2',
            author: 'Oleksandr',
            description: 'Фікс дублирования кнопок + очистка кешу'
        });
