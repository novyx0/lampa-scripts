(function() {
    setTimeout(function() {
        ...
        function updateButtons() {
            // Более надежная проверка существования уже обновленных кнопок
            let torrentSvg = $('.full-start__button.view--torrent svg').first();
            let onlineSvg = $('.full-start__button.view--online svg').first();
            let trailerSvg = $('.full-start__button.view--trailer svg').first();
            
            // Проверяем по viewBox - если он уже правильный, не обновляем
            if (torrentSvg.length && torrentSvg.attr('viewBox') === '0 0 50 50') {
                return;
            }

            // Очищаем старые SVG и добавляем новые
            $('.full-start__button.view--torrent').each(function() {
                let $button = $(this);
                if ($button.find('svg').attr('viewBox') !== '0 0 50 50') {
            ...
            $('.full-start__button.view--online').each(function() {
                let $button = $(this);
                if ($button.find('svg').attr('viewBox') !== '0 0 32 32') {
                    $button.find('svg').remove();
                    $button.prepend(`
            ...
            $('.full-start__button.view--trailer').each(function() {
                let $button = $(this);
                if ($button.find('svg').attr('viewBox') !== '0 0 80 70') {
                    $button.find('svg').remove();
                    $button.prepend(`
